...
import { Reward } from "@/lib/typescript";
import { cn } from "@/lib/utils";

export function GlowingEffectDemo({rewards}:{ rewards: Reward[]}) {
  ...
  return (
    <ul className="max-w-7xl w-full mx-auto grid grid-cols-1  md:grid-cols-3 gap-8 px-4 mt-12 md:mt-16">
     {rewards?.map((reward, index) => (
        <GridItem
          key={index}
          icon={reward.icon? <reward.icon size={24} /> : null} // Correct JSX syntax
          title={reward.title} // Assuming this should be the actual title text
...
const GridItem = ({icon, title, description, benefits, isLast}: Reward & { isLast: boolean }) => {
  return (
    <li className={cn("min-h-[14rem] list-none",{ "md:col-span-3": isLast })}>
        ...
        <div className="relative flex h-full flex-col justify-between gap-6 overflow-hidden rounded-xl border-0.75 p-6 shadow-[0px_0px_27px_0px_#2D2D2D]">
          <div className="relative flex flex-1 flex-col justify-between gap-3">
           ...
           {Array.isArray(benefits)  && <ul className="flex flex-col lg:flex-row gap-4">
            {benefits.map((value, index )=> <li key={index} className="flex-1 bg-zinc-900  p-3 rounded-lg border-[1px] border-zinc-800 "><h2 className="bg-gradient-to-r from-white to-gray-300 bg-clip-text text-transparent">
            {value} 
            </h2></li> )}
            </ul>}
          </div>
